...
public class WarehouseService : IWarehouseService
{
    ...
    public async Task<WarehouseDashboardData> GetDashboardDataAsync(CancellationToken cancellationToken = default)
    {
        ...
        var locationStats = warehouses.GroupBy(w => NormalizeLocation(w.Location, unspecifiedLocationLabel), locationComparer)
        ...
        var recentUnloadingCount = await _context.WarehouseUnloadings.AsNoTracking()
            .CountAsync(u => u.UnloadedAt >= thirtyDaysAgo, cancellationToken);

        var activeWarehouseIds = await _context.WarehouseUnloadings.AsNoTracking().Where(u => u.UnloadedAt >= activeThreshold)
            .Select(u => u.WarehouseId).Distinct().ToListAsync(cancellationToken);

        var activeWarehousesCount = activeWarehouseIds.Count;

        var monthlyVolumesRaw = await _context.WarehouseUnloadings.AsNoTracking()
        ...
        var topWarehouseRaw = await _context.WarehouseUnloadings.AsNoTracking().Where(u => u.UnloadedAt >= periodStart)
            .GroupBy(u => u.WarehouseId).Select(g => new
            {
                WarehouseId = g.Key,
            ...
            .OrderByDescending(x => x.TotalVolume).ThenByDescending(x => x.LastUnloadedAt).Take(5)
            .ToListAsync(cancellationToken);
        ...
        var topWarehouseStats = topWarehouseRaw
            .Select(item =>
            {
                ...
                return new TopWarehouseStat(item.WarehouseId, name, location,
                    Math.Round(item.TotalVolume, 2), item.TripCount, item.LastUnloadedAt);
            }).ToList();

        return new WarehouseDashboardData(warehouseDtos, totalWarehouses, distinctLocationCount,
            activeWarehousesCount, recentUnloadingCount, topLocationName, topLocationWarehouseCount,
            locationStats, topWarehouseStats, labels, values);
    }
    ...
    public async Task<WarehouseDetailsDto?> GetDetailsByIdAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        var unloadingDtos = unloadings.Select(u => new WarehouseUnloadingDto(
            u.Id, u.WarehouseId, u.ShipmentId, u.TruckPlate,
            u.UnloadedAt, u.UnloadedVolume, u.Notes, u.CreatedAt)).ToList();

        var shipmentOptions = await _context.Shipments.AsNoTracking()
            .Select(s => new ShipmentOptionDto(s.Id, $"{s.ReferenceNumber} - {s.Status}"))
            .ToListAsync(cancellationToken);
