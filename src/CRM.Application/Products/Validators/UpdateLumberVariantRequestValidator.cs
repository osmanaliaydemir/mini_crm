...
public class UpdateLumberVariantRequestValidator : AbstractValidator<UpdateLumberVariantRequest>
{
    private readonly IStringLocalizer<SharedResource> _localizer;

    public UpdateLumberVariantRequestValidator(IStringLocalizer<SharedResource> localizer)
    {
        _localizer = localizer;

        RuleFor(x => x.Id)
            .NotEmpty().WithMessage(_localizer["Validation_Product_Id_Required"]);
        ...
        RuleFor(x => x.RowVersion)
            .NotEmpty().WithMessage(_localizer["Validation_RowVersion_Required"]);
    }
}
